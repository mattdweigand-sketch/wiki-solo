...
3. **Entity types.** Show the user the 13 default types from [`workspaces/ingest/docs/schema.md`](workspaces/ingest/docs/schema.md) and ask which to keep. Default is all 13. Common drops: a B2C product may not need `customer` as named accounts; a research lab may not need `competitor`.
...
If you add custom entity types, also append a row for each to the "Entity Types" table in [`workspaces/ingest/docs/schema.md`](workspaces/ingest/docs/schema.md) so the ingest workflow knows about them.
...
- [`scripts/`](scripts/) — the backlink rebuild script (`rebuild_referenced_by.py`)
- [`.claude/commands/`](.claude/commands/) — Claude Code slash-command wrappers
- [`workspaces/ingest/workflows/`](workspaces/ingest/workflows/) — the 3-stage ingest pipeline (triage, extract, link)
- [`workspaces/*/CONTEXT.md`](workspaces/) — workspace task routers
- [`workspaces/maintenance/docs/`](workspaces/maintenance/docs/) — lint criteria, decision capture
- The schema's "Page Format," "Source-Type Summary Templates," and "Confidence Values" sections in [`schema.md`](workspaces/ingest/docs/schema.md) — domain-agnostic infrastructure
