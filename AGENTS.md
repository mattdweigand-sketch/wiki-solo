...
- `workflows/` - vendor-neutral prose workflows grouped into three workspaces, each with a `CONTEXT.md` entry point: `ingest/` (raw -> pages), `research/` (question -> answer), and `maintenance/` (lint, rotate-log, eval, artifact-promotion, capture, refresh-sourcing-queue, synthesize, review, export).
